...
public class ShuttlServerMBeanTest {
	...
	@Test(groups = { "fast-unit" })
	public void test_getShuttlHostName() throws ShuttlMBeanException {
		String hostname = this.serverMBean.getShuttlHostName();
		if (hostname.length() < 1)
			throw new ShuttlMBeanException("Invalid ShuttlHostName " + hostname);
	}

	@Test(groups = { "fast-unit" })
	public void test_addCluster() throws ShuttlMBeanException {
		this.serverMBean.addHadoopCluster(TEST_CLUSTER_NAME);
		this.serverMBean.setHadoopClusterHost(TEST_CLUSTER_NAME, TEST_CLUSTER_HOST);
		this.serverMBean.setHadoopClusterPort(TEST_CLUSTER_NAME, TEST_CLUSTER_PORT);
	...
	@Test(groups = { "fast-unit" }, expectedExceptions = ShuttlMBeanException.class)
	public void test_deleteCluster() throws ShuttlMBeanException {
		this.serverMBean.addHadoopCluster(TEST_CLUSTER_NAME);
		this.serverMBean.save();
		this.serverMBean.refresh();
		this.serverMBean.deleteHadoopCluster(TEST_CLUSTER_NAME);
