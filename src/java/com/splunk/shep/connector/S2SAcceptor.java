...
public class S2SAcceptor implements Acceptor {
	...
	public Channel handle(SocketChannel socket) throws Exception {
		logger.debug("New Connection on port=" + config.getPort() + " from " + socket.socket().getRemoteSocketAddress());
...
class S2SChannel implements Channel {
	...
	public void dataAvailable(byte[] buf, int offset, int len) throws AbortConnectionException {
		logger.debug("Received bytes=" + buf.length + " on port=" + config.getPort());
		try {
	...
	public void connectionClosed(SocketChannel socket) {
		logger.debug("Connection to port=" + config.getPort() + " from " + socket.socket().getRemoteSocketAddress().toString() + " closed.");
	}
}
