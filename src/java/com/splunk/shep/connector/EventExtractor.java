...
public class EventExtractor {
	...
	SelectionKey acceptKey = serverChannel.register(selector,
	...
	while (true) {
	    ...
	    if (keysAdded > 0) {
		logger.info("Selector returned " + keysAdded
		...
		if (i.hasNext()) {
			...
			EventChannel connection = new EventChannel(channel,
				targetIP, targetPort, id, debugging);
			connection.start();
...
class EventChannel extends Thread {
    ...
    public EventChannel(SocketChannel channel, String tarIP, int tarPort,
	    int id, boolean dbgFlag) {
	...
	try {
		...
		// signature portion: <sig 128 bytes> + <serverName 256 bytes> +
		// <mgmt port 16 bytes>
		...
		try {
		    do {
			int numBytes = sockIn.read(sigBuf, readLen,
				(SigSize - readLen));
			if (numBytes < 0)
			    throw (new IOException(
				    "WARN: lost client connection"));
		    ...
		    if (debugging)
			System.err.println("DEBUG: received Signature bytes: "
				+ sig + " " + sig.length());
		    sig = sig.replace('\0', ' ');
		    sig = sig.trim();
		    if (debugging)
			System.err.println("DEBUG: received s2s signature: "
				+ sig + " " + sig.length());

		    if ((sig.indexOf(TCPSIGNATURE_COOKED_SIG_V3)) >= 0) {
			logger.info("channel " + channelID + " signature "
				+ TCPSIGNATURE_COOKED_SIG_V3
				+ " not supported. Quit");
			break;
		    }
		    logger.info("channel " + channelID + " accept signature "
			    + sig);
		    ...
		    readLen = 0;
		    do {
			int numBytes = sockIn.read(svrNameBuf, readLen,
				(SvrNameSize - readLen));
			if (numBytes < 0)
			...
			readLen += numBytes;
			logger.debug("channel " + channelID + " received "
				+ readLen + " bytes");
		    } while (readLen < SvrNameSize);

		    String svrName = new String(svrNameBuf);
		    logger.debug("channel " + channelID + " Server Name: "
			    + svrName);

		    byte[] portBuf = new byte[PortSize];
		    readLen = 0;
		    do {
			int numBytes = sockIn.read(portBuf, readLen,
				(PortSize - readLen));
			if (numBytes < 0)
			    throw (new IOException("channel " + channelID
				    + " lost client connection"));
