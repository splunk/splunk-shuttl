...
/**
 * Each Acceptor port is handled in a Thread. Each channel's data handler can potentially block 
 * specially when it is trying to send data out. To make sure that each Acceptor port is independent,
 * i.e. it does not get affected by channel in other acceptor port being blocked, we need to have
 * each Selector(ConnectionManager) be run on its own thread. 
 */
class ConnectionManagerImpl extends Thread {
	...
	public ConnectionManagerImpl(Acceptor acceptor) throws IOException {
		...
		InetSocketAddress addr = new InetSocketAddress(InetAddress.getByName(acceptor.getBindIP()), acceptor.getPort());
			...
			while (it.hasNext()) {
				...
				if (key.isAcceptable()) {
					...
					try {
						...
						logger.debug("New connection on port=" + acceptor.getPort() + " from ip=" + socket.socket().getRemoteSocketAddress());
					} catch (Exception e) {
						socket.close();
						StringWriter sw = new StringWriter();
						e.printStackTrace(new PrintWriter(sw));
						logger.error("Failed to handle connection from client on port=" + acceptor.getPort() +
								" " + sw.toString());
					}
				}
				...
				if (key.isReadable()) {
					...
					if (bytesRead == -1) {
						logger.debug("Connection from ip=" + socket.socket().getRemoteSocketAddress() + " closed by sender.");
						...
						try {
							...
							// Channel handler does not want to continue with the connection. close it
							logger.warn("Connection closed by application. " + acex.getMessage());
							socket.close();
						}
						buf.clear();
					}
