...
public class S2SStateMachine {
	...
	public S2SStateMachine(S2SDataHandler callback) {
		initStateMachine(callback);
	}
	
	private void initStateMachine(S2SDataHandler callback) {
		State sigState = new SignatureReadingState();
		LengthReadingState lenReadingState = new LengthReadingState();
		RawDataReadingState rawDataReadingState = new RawDataReadingState(callback);
	...
	public void consume(byte[] buf, int startIndex, int bufLen) throws 
				InvalidSignatureException, InvalidS2SDataException, Exception {
...
interface State {
	...
	public boolean consume(StateContext ctxt)
					throws InvalidSignatureException, InvalidS2SDataException, Exception;
...
abstract class AbstractState implements State {
	...
	public boolean doConsume(StateContext ctxt, byte[] dest) {
		...
		if (need <= available) {
			System.arraycopy(ctxt.buf, ctxt.startIndex, dest, sizeDataReceived, need);
			ctxt.startIndex += need;
			sizeDataReceived += need;
			return false;
		} else {
			System.arraycopy(ctxt.buf, ctxt.startIndex, dest, sizeDataReceived, available);
...
class SignatureReadingState extends AbstractState {
	...
	private void verifySignature(byte[] signature) throws InvalidSignatureException {
		String sig = new String(signature, 0, 128);
		logger.debug("Verifying signature : " + sig);
		if (sig.startsWith(TCPSIGNATURE_COOKED_SIG_V3)) {
			throw new InvalidSignatureException("V3 protocol not supported at this time.");
			...
			throw new InvalidSignatureException("Unsupported signature " + sig + " received");
		}
	}
}
...
class LengthReadingState extends AbstractState {
	...
	public boolean consume(StateContext ctxt) throws InvalidS2SDataException {
		...
		if (!rv) {
			...
			if (size == 0) {
				throw new InvalidS2SDataException("Could not parse size of data properly");
			}
...
class RawDataReadingState extends AbstractState {
	private byte[] raw;
	private S2SDataHandler callback;
	public RawDataReadingState(S2SDataHandler callback) {
