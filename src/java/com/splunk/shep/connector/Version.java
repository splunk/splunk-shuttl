...
public class Version {
	...
	try {
	    ...
	    BUILD_INFO = P4_NUMBER + ":" + BUILD_DATE + ":" + BRANCH + ":"
		    + BUILD_MACHINE;
	} catch (Exception e) {
    ...
    public static boolean handleCli(String classname, String[] args,
	    OutputStream os) {
	...
	ps.print("Usage : java " + classname + " [" + OPT_VER + "]" + " ["
		+ OPT_FULLVER + "]" + " [" + OPT_HELP + "]");
    }
	...
	ps.print(String.format("%1$s %2$-" + (m - OPT_VER.length())
		+ "s - %3$s", OPT_VER, " ", MSG_VER)
		+ "\n"
		+ String.format("%1$s %2$-" + (m - OPT_FULLVER.length())
			+ "s - %3$s", OPT_FULLVER, " ", MSG_FULLVER)
		+ "\n"
		+ String.format("%1$s %2$-" + (m - OPT_HELP.length())
			+ "s - %3$s", OPT_HELP, " ", MSG_HELP) + "\n");
    }
	...
	ps.println("\n\n"
		+ String.format("%1$s %2$-" + (m - OPT_VER.length())
			+ "s - %3$s", OPT_VER, " ", MSG_VER)
		+ "\n"
		+ String.format("%1$s %2$-" + (m - OPT_FULLVER.length())
			+ "s - %3$s", OPT_FULLVER, " ", MSG_FULLVER)
    ...
    private static boolean handleHelp(String classname, String option,
	    PrintStream ps) {
