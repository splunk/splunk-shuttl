...
public class EventParser
{
	...
	private static final String KeySOURCETYPE = new String("MetaData:Sourcetype\0");
	...
	{
		if (debugging)
			System.err.println("DEBUG: channel " + channelID + " Receiving data size ...");
		
		int dataSize = -1;
		try{
			int rawlen = 4;
			byte[] raw_char = new byte[rawlen];
			//System.out.println("Initial byte array: " + Arrays.toString(raw_char));
			int readLen = 0;
			do {
				int numBytes = inStream.read(raw_char, readLen, (rawlen - readLen));
				if (numBytes < 0)
					throw (new IOException("channel " + channelID + " lost client connection"));
				
				readLen += numBytes;
				//String msg = Arrays.toString(raw_char); // new String(raw_char);
				//System.err.println("recvd size msg of " + readLen + " bytes so far: " + msg);
	...
	{
		logger.debug("channel " + channelID + "rReceiving data of " + size + " bytes ...");
	...
	/**
	 * Convenient method for extracting all the stuff from serialized CowPipelineData
	 * @param buf - buf is serialized CowPipelineData
	 */
	...
	public void processData(byte[] buf, int offset, int size, int count)
	{
		...
		if ( (!ignore) && hasAllData() )
		{
			...
			eventThruput.update(hostValue, sourceValue, sourceTypeValue, indexValue, size);
		}
		else
	...
	{
		//System.out.println("Parse field from " + offset + " of buffer size " + size );
		...
		if (key.equals(KeyRAW)) {
			...
			//rawBuffer = Arrays.copyOfRange(buf, offset+len, rawValue.length());
			hasRaw = true;
		}
			...
			if (sourceValue.equals("source::fwd-hb") || sourceValue.equals("source::fwd"))
				return -1; // drop hear-beat msg.
